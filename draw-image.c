...
static struct gpiod_line_request* gpiod_request_output_line(const char *path, uint32_t offset, bool active_low, enum gpiod_line_value value, const char *consumer)
{
...
int main(int argc, char **argv)
{
	...
	struct gpiod_line_request* io_reset = gpiod_request_output_line("/dev/gpiochip0", LINE_RESET, true, GPIOD_LINE_VALUE_INACTIVE, "display_reset");
	struct gpiod_line_request* io_dc = gpiod_request_output_line("/dev/gpiochip0", LINE_DC, false, GPIOD_LINE_VALUE_ACTIVE, "display_dc");
	...
	if (do_initialize)
	{
		...
		_spi_send_cmd(spi_fd, io_dc, 0x28, NULL, 0);

		gpiod_line_request_set_value(io_reset, LINE_RESET, GPIOD_LINE_VALUE_INACTIVE);
		usleep(1000);
		gpiod_line_request_set_value(io_reset, LINE_RESET, GPIOD_LINE_VALUE_ACTIVE);
		usleep(100000);
		gpiod_line_request_set_value(io_reset, LINE_RESET, GPIOD_LINE_VALUE_INACTIVE);
		usleep(250000);

		_spi_send_cmd(spi_fd, io_dc, 0x11, NULL, 0);
		usleep(130000);
		_spi_send_cmd(spi_fd, io_dc, 0x11, NULL, 0);
		usleep(130000);

		{
			...
			_spi_read_cmd(spi_fd, io_dc, 0x04, data, sizeof(data));
...

		{
			...
			uint8_t data = 0xa0;
			_spi_send_cmd(spi_fd, io_dc, 0x36, &data, sizeof(data));
		}

		{
			uint8_t data = 0x55;
			_spi_send_cmd(spi_fd, io_dc, 0x3a, &data, sizeof(data));
		}

		{
			uint8_t data[5] = { 0x0c, 0x0c, 0x00, 0x33, 0x33 };
			_spi_send_cmd(spi_fd, io_dc, 0xb2, data, sizeof(data));
		}

		{
			uint8_t data = 0x50;
			_spi_send_cmd(spi_fd, io_dc, 0xb7, &data, sizeof(data));
		}

		{
			uint8_t data = 0x2b;
			_spi_send_cmd(spi_fd, io_dc, 0xbb, &data, sizeof(data));
		}

		{
			uint8_t data = 0x2c;
			_spi_send_cmd(spi_fd, io_dc, 0xc0, &data, sizeof(data));
		}

		{
			uint8_t data = 0x01;
			_spi_send_cmd(spi_fd, io_dc, 0xc2, &data, sizeof(data));
		}

		{
			uint8_t data = 0x0b;
			_spi_send_cmd(spi_fd, io_dc, 0xc3, &data, sizeof(data));
		}

		{
			uint8_t data = 0x20;
			_spi_send_cmd(spi_fd, io_dc, 0xc4, &data, sizeof(data));
		}

		{
			uint8_t data = 0x0f;
			_spi_send_cmd(spi_fd, io_dc, 0xc6, &data, sizeof(data));
		}

		{
			uint8_t data[2] = { 0xa4, 0xa1 };
			_spi_send_cmd(spi_fd, io_dc, 0xd0, data, sizeof(data));
		}

		{
			uint8_t data[14] = { 0xd0, 0x00, 0x02, 0x07, 0x0b, 0x1a, 0x31, 0x54, 0x40, 0x29, 0x12, 0x12, 0x12, 0x17 };
			_spi_send_cmd(spi_fd, io_dc, 0xe0, data, sizeof(data));
		}
		...
		_spi_send_cmd(spi_fd, io_dc, 0x29, NULL, 0);

		{
			...
			_spi_send_cmd(spi_fd, io_dc, 0x2a, data, sizeof(data));
		}

		{
			...
			_spi_send_cmd(spi_fd, io_dc, 0x2b, data, sizeof(data));
		}
	}

	_spi_send_cmd(spi_fd, io_dc, 0x2c, NULL, 0);
	...
	if (do_test)
	{
		// black
		{
			...
			_spi_transfer16(spi_fd, fb, fb, ARRAY_SIZEOF(fb));
		}
